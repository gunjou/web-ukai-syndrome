...
const ListPesertaModal = ({ idTarget, namaTarget, onRefresh }) => {
  ...
  const fetchKelas = async () => {
    ...
    setLoading(true);
    try {
      const res = await Api.get(`/batch/peserta/${idTarget}`);
  ...
  const handleDelete = (idUserBatch) => {
    ConfirmToast(`Yakin ingin menghapus peserta untuk batch ini?`, async () => {
      try {
        await Api.delete(`/batch/peserta/${idUserBatch}`);
        toast.success(`Peserta untuk batch ini berhasil dihapus.`);
        fetchKelas();
        onRefresh?.();
      } catch (err) {
    ...
    <div className="relative">
      <div className="flex justify-between items-center mb-3">
        <h2 className="text-lg font-bold">
          Daftar Peserta Batch - {namaTarget}
        </h2>
      </div>
                ...
                <tr key={index} className="bg-gray-100 hover:bg-gray-300">
                  ...
                  <td className="border px-3 py-2 text-xs sm:text-sm">
                    <div className="relative group">
                      <button
                        onClick={() => handleDelete(peserta.id_userbatch)}
...
export default ListPesertaModal;
